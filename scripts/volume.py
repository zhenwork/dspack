...
def volume_symmetrize(volume_3d, volume_mask_3d=None, threshold_keep=(-100,1000), operator="+++,+-+,---,-+-"):
    """
    # checked +1 
    """
    ...
    volumeSym = np.zeros(volume.shape)
    weightSym = np.zeros(volume.shape)
    if not isinstance(operator,str):
        return None,None
    for operator_str in operator.split(","):
        if operator_str.strip() == "+++":
            volumeSym += volume
            weightSym += volumeMask
        elif operator_str.strip() == "++-":
            volumeSym += volume[:,:,::-1]
...
@jit
def volume_hkl_reshape(volume,astar,bstar,cstar,volume_mask=None,\
                       center=None,volnew_size=None,volnew_center=None,\
                       voxel_new_size=1.,threshold_keep=(-100,1000)):
    """
    # checked +1
    """
    nx,ny,nz = volume.shape 
    if center is None:
        dcx,dcy,dcz = (nx-1)/2,(ny-1)/2,(nz-1)/2
    else:
        dcx,dcy,dcz = center
        
    if volnew_center is None:
        ncx,ncy,ncz = dcx,dcy,dcz
    else:
        ncx,ncy,ncz = volnew_center
        
    if volnew_size is None:
        nnx,nny,nnz = nx,ny,nz
    else:
        nnx,nny,nnz = volnew_size
    ...
    volnew = np.zeros((nnx,nny,nnz))
    weight = np.zeros((nnx,nny,nnz))
    ...
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                ...
                hkl = float(i-dcx)*astar + float(j-dcy)*bstar + float(k-dcz)*cstar 
                ...
                # throw one line more
                if (x < 0) or x > (nnx-2) or (y < 0) or y > (nny-2) or (z < 0) or z > (nnz-2):
                    continue
...
@jit
def volume_radial_profile(volume_3d, volume_mask_3d=None,volume_center=None,threshold_keep=(-100,1000),\
                    contribute_window=5, expand_scale=4, basis_by_column=None):
    """
    # checked +1
    """
    volume = volume_3d.copy()
    ...
    if volume_center is None:
        volume_center = ((nx-1)/2,(ny-1)/2,(nz-1)/2)
    ...
    radius_assign = np.around(radius).astype(int)   # rounded integers

    max_radius = int(np.amax(radius_assign))

    radius_1d = np.zeros(max_radius+1)

    weight_1d = np.zeros(radius_1d.shape)
    marker_1d = np.zeros(radius_1d.shape)

    half_window = int((contribute_window-1.)/2.)


    min_radius_has_value = int(round(np.amin(radius_assign[volumeMask==1])))
    max_radius_has_value = int(round(np.amax(radius_assign[volumeMask==1])))


    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                r = radius[i,j,k]  # raw radius, not rounded
    ...
    radius_1d[:(min_radius_has_value)] = 0
    radius_1d[(max_radius_has_value+1):] = 0
...
def correlation(HKLI_1, HKLI_2, center=None, lattice_constant_A_deg=None, res_shell_step_Ainv=None, \
                num_res_shell=15, res_cut_off_high_A=1.4, res_cut_off_low_A=50, num_ref_uniform=True, \
                vmin=-100, vmax=1000):
...
def volume_to_txt(volume,volume_center=None,fsave="tmp.txt", vmask=None, \
...
def volume_to_phenix(volume,volume_center=None,fsave="tmp.txt", vmask=None, \
                    threshold_keep=(-100,1000), lift=0, headers=None):
    """
    # checked +1
    """
