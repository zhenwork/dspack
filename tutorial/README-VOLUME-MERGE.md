...
- ```--fname```:  
The file name that contains your data files. It's usually the result of [```dsimage.pca```](https://github.com/zhenwork/dspack/blob/main/tutorial/README-IMAGE-PCA.md), which is ```cleaned_data_scaled_with_pca.dsdata```.

- ```--fsave```:  
The file name to save the result of ```dsmap.merge```, the default is the the same as ```--fname```.
...
- ```--select_image_idx```:  
The selected image index to merge, for example, ```--select_image_idx 50-100,200-300``` means merging images with index from ```[50,100]``` and ```[200,300]```, inclusively. 
